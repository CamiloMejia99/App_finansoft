...
namespace FNTC.Framework.Converters.Numbers
{
    public static class NumerosALetras
    {
        ...
        "diecisiete ", "dieciocho ", "diecinueve", "veinti", "treintai", "cuarentai",
        ...
        public static String Convertir(String numero, bool mayusculas)
        {
            ...
            if (mc.Count > 0)
            {
                ...
                parte_decimal = "con " + Num[1] + "ctvos.";
                //se convierte el numero a literal
                if (long.Parse(Num[0]) == 0)
                {//si el valor es cero                
                    literal = "cero ";
                }
                else if (long.Parse(Num[0]) > 999999)
                {//si es millon
                    literal = getMillones(Num[0]);
                }
                else if (long.Parse(Num[0]) > 999)
                {//si es miles
                    literal = getMiles(Num[0]);
                }
                else if (long.Parse(Num[0]) > 99)
                {//si es centena
                    literal = getCentenas(Num[0]);
                }
                else if (long.Parse(Num[0]) > 9)
                {//si es decena
        ...
        {// 999 o 099
            ...
            {//es centena
                ...
                {
                    var centenas = CENTENAS[int.Parse(num.Substring(0, 1))];
                    var decenas = getDecenas(num.Substring(1));
