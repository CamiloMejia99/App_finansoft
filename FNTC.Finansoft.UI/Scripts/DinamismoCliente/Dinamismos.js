        ...
        },
        {
            ...
            action: function () {
                if (table.rows({ selected: true }).count() == 1) {
                    ...
                    $("#elimin").attr("href", "/Aportes/Aportes/Delete?id=" + table.rows({ selected: true }).data()[0]['nit']);
                } else {
                    Swal.fire({ title: "Recuerda", text: "Primero selecciona un registro", confirmButtonText: "Lo recordare" });
                }
            }
        ...
        if ($("#idTipoCuotaCalculo").val() == 0) {
            ...
            if (!$("#valor").prop("disabled")) {
                if (parseFloat(quitarFormato($("#valorCuota").val())) < parseFloat(quitarFormato($("#SaldoMinimo").val()))) {
                    ...
                    Swal.fire({
                        title: 'Estas seguro?',
                        text: "Las fichas de aportes se modificaran, está seguro de continuar con la operación?",
                        icon: 'warning',
...
function AhorrosPermanentes() {
    ...
    var botones = [
...

        {
            ...
            action: function () {
                if (table.rows({ selected: true }).count() == 1) {
                    //modal editar
                    $("#EditarAfiliarAhorros").modal("show");

                    $("#EditformAfiliacion").attr("action", "/Ahorros/Ahorros/EditarFAP");
                    //option').prop
                    console.log(table.rows({ selected: true }).data()[0]["valor"]);
                    $("#EditPersonaId").val(table.rows({ selected: true }).data()[0]["nit"]);
                    $("#nomPersona").text(table.rows({ selected: true }).data()[0]["nombres"]);
                    $("#empresa").val(table.rows({ selected: true }).data()[0]["empresa"]);
                    $("#agencia").val(table.rows({ selected: true }).data()[0]["oficina"]);
                    $("#tipoPago").val(table.rows({ selected: true }).data()[0]["tipoPago"]);
                    $("#porcentaje").val(table.rows({ selected: true }).data()[0]["porcentaje"]);
                    $("#EditvalorCuota").val(table.rows({ selected: true }).data()[0]["valor"]);
                    $("#EditV_cuota").val(table.rows({ selected: true }).data()[0]["valor"]);
                    $("#activo").val(table.rows({ selected: true }).data()[0]["activo"].toString());
                    $("#EditTasa_interes").val(table.rows({ selected: true }).data()[0]["Tasa_interes"].toString());
    ...
    /*table.columns(14).visible(false); table.columns(14).visible(false);*/
    //FIN DATATABLES
