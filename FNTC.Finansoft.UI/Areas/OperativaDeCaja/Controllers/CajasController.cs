        ...
        public ActionResult Create([Bind(Include = "Codigo_caja,Nombre_caja,Consecutivo_ini,consecutivo_fin,TopeMaximo_caja,consecutivo_actual,Serie,agencia,cta_abastecimiento")] Caja caja)
        {
			...
			else
			{
				...
				else
				{
					...
					CuentaMayor plan = db.PlanCuentas.Find(id);
					if (plan == null)
					{
						return HttpNotFound();
					}
					else
					{
						...
						else
						{
							if (ModelState.IsValid)
							{
								...
								var insertAbastecimientos = new RegistroAbastecimiento()
								{
									fecha = DateTime.Now,
									cod_caja = caja.Codigo_caja,
									cta_abastecimiento = caja.cta_abastecimiento,
									abastecimiento = Convert.ToDecimal(caja.TopeMaximo_caja),
									agencia = Convert.ToInt32(caja.agencia)

								};
								db.RegistroAbastecimientos.Add(insertAbastecimientos);
