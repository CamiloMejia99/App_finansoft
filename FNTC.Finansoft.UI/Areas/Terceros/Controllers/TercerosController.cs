...
using FNTC.Finansoft.Accounting.BLL.Terceros;
...
namespace FNTC.Finansoft.UI.Areas.Terceros.Controllers
{
    ...
    public class TercerosController : Controller
    {
        ...
        [HttpPost]
        public JsonResult GetTercerosAutocopletar2(string cadena)
        {
            var cad = (from x in db.Terceros
        ...
        public ActionResult GetTerceros(string term = "", string outputFormat = "datatables")
        {
            ...
            switch (outputFormat)
            {
                case "datatables":
                    ...
                    => new[] { x.NIT,  x.NombreComercial+x.NOMBRE1 + " "+ x.NOMBRE2 +" "+  x.APELLIDO1 +" "+ x.APELLIDO2,    x.DIR,  x.TEL,  x.TELMOVIL,  x.EMAIL ,BotonEditar(x.NIT),BotonEliminar(x.NIT)
                    });
        ...
        public ActionResult ListaCargos()
        {
            ...
            {
                return PartialView(db.CargoEmpresaTer.ToList());
            }
        }
        public ActionResult ListaCargosEditar(string nit)
        {
            using (var db = new AccountingContext())
            {
                ViewBag.nit = nit;
                ViewBag.cargo = new TercerosBLL().ObtenerCargo(nit);
                var cargoNom = new TercerosBLL().ObtenerCargo(nit);
                ViewBag.CargoNombre = new TercerosBLL().ObtenerCargoNombre(cargoNom);
                var nivelSelecccionado = new TercerosBLL().ObtenerNivel(nit);
                ViewBag.NivelNombre = new TercerosBLL().ObtenerNivelNombre(nivelSelecccionado);
                return PartialView(db.CargoEmpresaTer.ToList());
            }
        }
        ...
        public ActionResult Edit(string nit)
        {
            ...
            */
            ViewBag.nit = nit;
            ViewBag.Nivel = new TercerosBLL().ObtenerNivel(nit);
            var nivelSelecccionado = new TercerosBLL().ObtenerNivel(nit);
            ViewBag.NivelNombre = new TercerosBLL().ObtenerNivelNombre(nivelSelecccionado);
        ...
        [HttpPost]
        public JsonResult verificaExisteTercero(string nit)
        {
            return new JsonResult { Data = new { status = db.Terceros.Where(x => x.NIT == nit).Any() } };
        }

        public static string GetNombreTercero(Tercero tercero)
        { 
            return tercero.NombreComercial+tercero.NOMBRE1+" "+tercero.NOMBRE2+" "+tercero.APELLIDO1+" "+tercero.APELLIDO2;
        }

        public static int GetAntiguedadMeses(Tercero tercero)
        { 
            int result = 0;
            try
            {
                DateTime FechaActual = FechaLocal.GetFechaColombia();
                result = tercero.FECHAEXP != null ? FechaLocal.DateTimeSpan.CompareDates((DateTime)tercero.FECHAR, FechaActual).Months : 0;
            }
            catch (Exception ex)
            {
            }
